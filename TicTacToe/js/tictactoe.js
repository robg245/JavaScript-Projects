...
if (!selectedSquares.some(element => element.includes(squareNumber))) {
    ...
    if (activePlayer === 'X')   {
        ...
        select.style.backgroundImage = 'url("images/red_X.png")';
        //Active player may only be 'X' or 'O' so, if not 'X' it must be 'O'
         }
         else   {
             //IF activePlayer is equal to 'O', the o.png is placed in HTML
             select.style.backgroundImage = 'url("images/purple_O.png")';
         }
         //square number and active player are concatenated together and added to array.
selectedSquares.push(squareNumber + activePlayer );
//This calls a function to check for any win conditions.
checkWinConditions ();
//THis condition is for anything changing the active player.
if (activePlayer === 'X')   {
    //IF activePlayet is 'X' change it to 'O'.
    activePlayer = 'O';
...
function checkWinConditions()   {
  ...
  // X 1,4,7 condtion.
  else if (arrayIncludes ('1X', '4X','7X' )) { drawWinLine(304, 50, 304, 58)}
  // X 2,5,8
